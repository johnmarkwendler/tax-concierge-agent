...
def agent_app(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("GOOGLE_CLOUD_PROJECT", "gen-lang-client-0762290274")
    monkeypatch.setenv("GOOGLE_CLOUD_LOCATION", "us-east1")

    from tax_concierge_agent.agent_runtime_app import LocalStructuredLogger, agent_runtime

    agent_runtime.logger = LocalStructuredLogger()
...
async def test_agent_stream_query(agent_app) -> None:
    ...
    """
    if os.environ.get("RUN_LIVE_AGENT_RUNTIME_TESTS") != "TRUE":
        pytest.skip("Set RUN_LIVE_AGENT_RUNTIME_TESTS=TRUE to run live model streaming.")
